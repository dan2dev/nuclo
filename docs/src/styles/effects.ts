...
export const fx = {
  ...
  gradientText: css({ display: "inline-block", backgroundImage: "linear-gradient(94deg, var(--c-primary-hover) 4%, var(--c-accent-cool) 96%)", backgroundClip: "text", color: "transparent", raw: { "-webkit-background-clip": "text", "-webkit-text-fill-color": "transparent" } }),
  ...
  gradientBorder: css({ border: "1px solid transparent", backgroundImage: "linear-gradient(var(--c-bg-card), var(--c-bg-card)), linear-gradient(155deg, rgba(45,212,191,0.5) 0%, var(--c-border) 32%, var(--c-border) 68%, rgba(56,189,248,0.42) 100%)", raw: { "background-origin": "border-box", "background-clip": "padding-box, border-box" } }),
  demoElevated: css({ boxShadow: "var(--card-glow)" }),
  brandLogo: css({ filter: "var(--logo-filter, none)" }),
};
