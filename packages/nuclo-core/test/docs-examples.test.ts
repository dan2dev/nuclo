...
describe("Documentation Examples", () => {
  ...
  describe("API - when() Conditional Rendering", () => {
    it("should render content based on boolean condition", () => {
      ...
      const app = div(
        when(() => isLoggedIn, div("Welcome back!")).else(div("Please log in")),
      );
    ...
    it("should support chaining multiple conditions", () => {
      ...
      const app = div(
        when(() => role === "admin", div("Admin Panel"))
          .when(() => role === "user", div("User Dashboard"))
          .else(div("Guest View")),
      );
  ...
  describe("Styling - createBreakpoints", () => {
    ...
    it("should support responsive breakpoints", () => {
      ...
      const result = cn(padding("1rem").fontSize("14px"), {
    ...
    it("should apply styles to elements", () => {
      ...
      const buttonStyle = cn(
        bg("#3b82f6").color("white").padding("0.5rem 1rem"),
      );
  ...
  describe("Styling - Style Helpers", () => {
    ...
    it("should support grid() shorthand", () => {
      ...
      expect(cssStyles).toHaveProperty(
  ...
  describe("Examples - Todo List", () => {
    it("should render todo list with full functionality", () => {
      ...
      const app = div(
        ...
        ).else(p({ className: "empty-state" }, "No todos yet! Add one above.")),
      );
      ...
      expect(document.querySelector(".todo-text")?.textContent).toBe(
        "Test task",
      );
        ...
        return div(
          ...
          when(
            ...
            div(
              ...
              list(
                () => task.subtasks,
                (subtask) => TaskItem(subtask, depth + 1),
              ),
            ),
          ),
        );
        ...
        when(
          ...
          .when(
            ...
            div(
              list(
                () => state.data,
                (item) => div({ className: "item" }, item.name),
              ),
            ),
          )
          .else(div({ className: "empty" }, "No data loaded")),
      );
  ...
  describe("Styling - Complete Styling Example", () => {
    it("should work with organized styles object pattern", () => {
      ...
      expect(document.querySelector("p")?.textContent).toBe(
        "This is styled with nuclo CSS-in-JS",
      );
    ...
    it("should work with responsive styles", () => {
      ...
      const cardGrid = cn(grid().gridTemplateColumns("1fr").gap("1.5rem"), {
