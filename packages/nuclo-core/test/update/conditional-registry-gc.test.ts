 ...
 *
 * Targets uncovered lines in src/update/registry.ts:
 *
...
describe('storeConditionalInfo', () => {
  ...
  it('overwrites existing info for the same node', () => {
    const node = document.createElement('div');
    const info1 = { condition: () => true, tagName: 'div' as ElementTagName, modifiers: [], isSvg: false };
    const info2 = { condition: () => false, tagName: 'span' as ElementTagName, modifiers: [], isSvg: false };
...
describe('getActiveConditionalNodes – GC simulation (lines 53, 60)', () => {
  ...
  it('returns live nodes when WeakRef is working normally', () => {
    ...
    const info = { condition: () => false, tagName: 'article' as ElementTagName, modifiers: [], isSvg: false };
    storeConditionalInfo(node, info);
  ...
  it('handles mix of dead and live refs in same pass', () => {
    ...
    storeConditionalInfo(deadNode, { condition: () => true, tagName: 'div' as ElementTagName, modifiers: [], isSvg: false });

    // Restore and register a live node
    vi.stubGlobal('WeakRef', OriginalWeakRef);
    const liveNode = document.createElement('span');
    storeConditionalInfo(liveNode, { condition: () => true, tagName: 'span' as ElementTagName, modifiers: [], isSvg: false });
