...
describe("tags arrays", () => {
  ...
  describe("svgTags", () => {
    ...
    it("should have some overlap with HTML htmlTags but be distinct", () => {
      ...
      const sharedTags = svgTags.filter((tag) =>
        htmlTags.includes(tag as (typeof htmlTags)[number]),
      );
      ...
      const uniqueSvgTags = svgTags.filter(
        (tag) => !htmlTags.includes(tag as (typeof htmlTags)[number]),
      );
  ...
  describe("array relationships", () => {
    it("should have no overlap between selfClosingTags and non-self-closing htmlTags", () => {
      ...
      const overlap = selfClosingTags.filter((tag) =>
        nonSelfClosingTags.includes(tag),
      );
    ...
    it("should have selfClosingTags + non-self-closing htmlTags equal total HTML htmlTags", () => {
      ...
      expect(selfClosingTags.length + nonSelfClosingTags.length).toBe(
        htmlTags.length,
      );
    });
  });
