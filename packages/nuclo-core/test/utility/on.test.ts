...
describe("utility/on (event listener helper)", () => {
  ...
  it("works with custom events and typed detail", () => {
    ...
    const boxBuilder = (globalThis as any).div(
      on<"my-event", CustomEvent<Detail>>("my-event", handler),
    );
    const box = boxBuilder(document.body as any, 0) as HTMLDivElement;
    document.body.appendChild(box);

    box.dispatchEvent(
      new CustomEvent<Detail>("my-event", { detail: { value: 42 } }),
    );
    box.dispatchEvent(
      new CustomEvent<Detail>("my-event", { detail: { value: 7 } }),
    );
