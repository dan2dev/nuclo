...
describe("utility/resolveRenderable", () => {
  ...
  it("returns the element itself when result is already an element", () => {
    const produced = (globalThis as any).span("Inside")(
      host as any,
      0,
    ) as HTMLElement;
    // Sanity
  ...
  it("returns null for non-function, non-element values", () => {
    ...
    expect(
      resolveRenderable({ notTag: true } as any, host as any, 0),
    ).toBeNull();
  });

  it("propagates errors thrown by a NodeModFn", () => {
    const err = new Error("boom");
    const nodeModFn: NodeModFn<any> = () => {
      throw err;
    };
    expect(() => resolveRenderable(nodeModFn, host as any, 0)).toThrowError(
      err,
    );
  });
