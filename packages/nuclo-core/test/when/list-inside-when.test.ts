...
function findCommentIndex(container: HTMLElement, needle: string): number {
  return Array.from(container.childNodes).findIndex(
    (n) =>
      ...
      (n as Comment).textContent?.includes(needle),
  );
}
...
describe("List Inside When", () => {
  ...
  it("updates list while condition stays true", () => {
    ...
    const values = Array.from(container.querySelectorAll("[data-id]")).map(
      (n) => n.textContent,
    );
  ...
  it("removes list when condition becomes false and restores later with new data", () => {
    ...
    const rendered = Array.from(container.querySelectorAll("[data-id]")).map(
      (n) => n.textContent,
    );
  ...
  it("list in else branch works and toggles correctly", () => {
    const whenFn = when(() => condition, "Shown").else(
      list(
        () => items,
        (item: Item) => {
