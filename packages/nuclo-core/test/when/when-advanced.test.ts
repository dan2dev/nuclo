...
describe("when (advanced scenarios)", () => {
  ...
  describe("multiple element branches", () => {
    it("renders multiple sibling elements in matching when branch", () => {
      ...
      expect(container.querySelector(".fallback-a")?.textContent).toBe(
        "fallback A",
      );
      expect(container.querySelector(".fallback-b")?.textContent).toBe(
        "fallback B",
      );
    });
    ...
    it("updates only inner when when outer condition unchanged", () => {
      ...
      const block = when(
        ...
        div(
          ...
          ).else(span("inner-no", { id: "inner-no" })),
        ),
      ).else(div("outer hidden", { id: "outer-hidden" }));
      ...
      expect(container.querySelector("#inner-no")?.textContent).toBe(
        "inner-no",
      );
      ...
      expect(container.querySelector("#inner-yes")?.textContent).toBe(
        "inner:1",
      );
  ...
  describe("list inside when (advanced)", () => {
    ...
    it("handles list + plain elements together inside a branch", () => {
      ...
      expect(
        Array.from(container.querySelectorAll("[data-n]")).map(
          (n) => n.textContent,
        ),
      ).toEqual(["n:1", "n:2"]);
  ...
  describe("multi-group switching correctness", () => {
    it("cleans previous group and renders next group exactly once", () => {
      ...
      const block = when(
        ...
        .when(() => mode === "b", h1("Mode B", { id: "b1" }))
        .when(
          () => mode === "c",
          h1("Mode C", { id: "c1" }),
          h2("C2", { id: "c2" }),
        )
        .else(h1("Fallback", { id: "fb" }));
