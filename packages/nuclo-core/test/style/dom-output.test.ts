...
describe("Style utilities - DOM Output", () => {
  ...
  it("should apply className to div element when using breakpoint modifier as child", () => {
    ...
    const header = div(styles.header, "Header Content")();
    ...
    expect(
      headerClassName.split(" ").filter((c) => c.startsWith("n")).length,
    ).toBe(1); // Only one nuclo class
  ...
  it("should correctly render breakpoint className in the DOM", () => {
    ...
    expect(
      elementClassName.split(" ").filter((c) => c.startsWith("n")).length,
    ).toBe(1); // Only one nuclo class
