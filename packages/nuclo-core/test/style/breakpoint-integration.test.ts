...
describe("Style Queries Integration", () => {
  ...
  it("should work as a modifier when passed to an element", () => {
    ...
    expect(className).toMatch(/n[a-f0-9]{8}/);
    expect(className.split(" ").filter((c) => c.startsWith("n")).length).toBe(
      1,
    ); // Only one nuclo class
  ...
  it("should create CSS rules in the style sheet", () => {
    ...
    const smallStyleRules = Array.from(smallMediaRule.cssRules).filter(
      (rule) => rule.type === CSSRule.STYLE_RULE,
    );
    const largeStyleRules = Array.from(largeMediaRule.cssRules).filter(
      (rule) => rule.type === CSSRule.STYLE_RULE,
    );
  ...
  it("should create media queries with prefixed class names", () => {
    ...
    const mediaStyleRules = Array.from(
      largeMediaRule.cssRules,
    ) as CSSStyleRule[];

    // Check that the class name exists in the media query
    const largeRule = mediaStyleRules.find(
    ...
    expect(smallMediaRule).toBeTruthy();
    const smallMediaStyleRules = Array.from(
      smallMediaRule.cssRules,
    ) as CSSStyleRule[];

    // Check that the same class name exists in the small media query
    const smallRule = smallMediaStyleRules.find(
      (rule) => rule.selectorText === `.${className}`,
    );
  ...
  it("should handle the exact user scenario with theme colors", () => {
    ...
    const mediaStyleRules = Array.from(
      mediumMediaRule.cssRules,
    ) as CSSStyleRule[];
    const mediumRule = mediaStyleRules.find(
      (rule) => rule.selectorText === `.${className}`,
    );
  ...
  describe("Two-argument signature: default styles + breakpoints", () => {
    it("should accept default styles as first argument and breakpoints as second", () => {
      ...
      const result = cn(bg("#00FF00").fontSize("16px"), {
    ...
    it("should apply default styles without media query", () => {
      ...
      const defaultRule = styleRules.find((rule) =>
        rule.selectorText.match(/^\.n[a-f0-9]{8}$/),
      );
    ...
    it("should match user example: default width(100%) + breakpoint width(50%)", () => {
      ...
      const result = cn(bg("#FF0000").width("100%"), {
        medium: width("50%"),
      });
      ...
      const defaultRule = styleRules.find(
        (rule) => rule.selectorText === `.${className}`,
      );
      expect(defaultRule).toBeTruthy();
      if (defaultRule) {
  ...
  describe("createStyleQueries with @container prefix", () => {
    it("should create container queries with @container prefix", () => {
      ...
      const rules = Array.from(styleSheet?.sheet?.cssRules || []);
      const containerRules = rules.filter(
        (rule) => rule instanceof CSSContainerRule,
      );
  ...
  describe("createStyleQueries with @supports prefix", () => {
    it("should create feature queries with @supports prefix", () => {
      ...
      const rules = Array.from(styleSheet?.sheet?.cssRules || []);
      const supportsRules = rules.filter(
        (rule) => rule instanceof CSSSupportsRule,
      );
  ...
  describe("createStyleQueries with mixed query types", () => {
    it("should support mixing @media, @container, and @supports queries", () => {
      ...
      const result = cn(bg("#000000").width("100%"), {
        small: bg("#FF0000"),
        medium: bg("#00FF00"),
  ...
  describe("Pseudo-class support", () => {
    it("should automatically support pseudo-classes like :hover without defining them", () => {
      ...
      const result = cn(bg("#000000").color("#FFFFFF"), {
      ...
      // Find the hover rule
      const hoverRule = styleRules.find((rule) =>
    ...
    it("should support multiple pseudo-classes automatically", () => {
      ...
      const focusRule = styleRules.find((rule) =>
        rule.selectorText.includes(":focus"),
      );
      const activeRule = styleRules.find((rule) =>
    ...
    it("should support mixing pseudo-classes with media queries", () => {
      ...
      const result = cn(bg("#000000").fontSize("14px"), {
        small: fontSize("15px"),
