...
describe("named classNames with cn()", () => {
  ...
  it("uses the provided class name for default styles", () => {
    ...
    const rule = rules.find(
      (cssRule) => cssRule.selectorText === `.${result.className}`,
    );
  ...
  it("uses the provided class name when only query styles are passed", () => {
    ...
    const rule = rules.find(
      (cssRule) => cssRule.selectorText === `.${result.className}:hover`,
    );
    expect(rule).toBeTruthy();
  ...
  it("uses the provided class name for default styles and media queries", () => {
    ...
    const result = cn("named-responsive-card", width("100%").padding("8px"), {
      small: width("50%"),
    });
    ...
    const baseRule = rules.find(
      (cssRule): cssRule is CSSStyleRule =>
        ...
        cssRule.selectorText === `.${result.className}`,
    );
    ...
    const mediaRule = rules.find(
      (cssRule): cssRule is CSSMediaRule =>
        ...
        cssRule.media.mediaText === "(max-width: 600px)",
    );
    expect(mediaRule).toBeTruthy();

    const responsiveRule =
      mediaRule &&
      Array.from(mediaRule.cssRules).find(
        (cssRule): cssRule is CSSStyleRule =>
          cssRule instanceof CSSStyleRule &&
          cssRule.selectorText === `.${result.className}`,
      );
  ...
  it("avoids collisions when the same named class receives different styles", () => {
    ...
    const redRule = rules.find(
      (cssRule) => cssRule.selectorText === `.${redTitle.className}`,
    );
    const blueRule = rules.find(
      (cssRule) => cssRule.selectorText === `.${blueTitle.className}`,
    );
