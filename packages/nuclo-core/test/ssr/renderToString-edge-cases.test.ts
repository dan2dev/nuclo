...
describe("SSR renderToString - Edge Cases", () => {
  ...
  describe("attribute edge cases", () => {
    it("should handle null attributes", () => {
      const html = renderToString(div({ id: null, class: "test" }, "Content"));
  ...
  describe("nested structures", () => {
    it("should handle deeply nested elements", () => {
      const html = renderToString(div(div(div(div(span("Deep content"))))));

      expect(html).toContain(
        "<div><div><div><div><span><!-- text-0 -->Deep content</span></div></div></div></div>",
      );
    });
