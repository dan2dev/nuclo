...
describe("Hydration — when() and list()", () => {
  ...
  describe("when()", () => {
    ...
    describe("when() with multiple content nodes per branch", () => {
      it("renders all nodes of the active branch", () => {
        ...
        injectSsr('<div><h2>Please log in</h2><p>Enter your credentials</p></div>');
        const liveDiv = render(component, container);
    ...
    describe("when() with reactive text inside branches", () => {
      it("reactive text inside the active branch updates correctly", () => {
        ...
        let label = "hello";
        const component = div(
          when(() => visible, span(() => label)).else(span("hidden"))
        );
  ...
  describe("list()", () => {
    ...
    describe("DOM element identity is preserved across updates", () => {
      it("reorder — existing elements are moved, not recreated", () => {
        ...
        const component = ul(
          list(() => tasks, task =>
            li({ "data-id": String(task.id) }, task.name)
          )
        );
      ...
      it("existing elements survive a partial update that adds new items", () => {
        ...
        let items: Item[] = [{ id: 1, label: "first" }, { id: 2, label: "second" }];
        const component = ul(
          list(() => items, item => li({ "data-id": String(item.id) }, item.label))
        );

        injectSsr('<ul><li data-id="1">first</li><li data-id="2">second</li></ul>');
        const liveUl = render(component, container);
        ...
        expect(liveUl.querySelector('[data-id="3"]')!.textContent).toBe('third');
      });
    });
    ...
    describe("list() with object items and reactive text", () => {
      it("renders object fields as text content", () => {
        ...
        const component = ul(
          list(() => users, u => li({ "data-id": String(u.id) }, u.name))
        );

        injectSsr('<ul><li data-id="1">Alice</li><li data-id="2">Bob</li></ul>');
        const liveUl = render(component, container);

        expect(liveUl.querySelector('[data-id="1"]')!.textContent).toBe('Alice');
        ...
        expect(liveUl.querySelectorAll('li').length).toBe(3);
        expect(liveUl.querySelector('[data-id="3"]')!.textContent).toBe('Carol');
      });
      ...
      it("reactive text inside list items updates when item data changes", () => {
        ...
        expect(liveUl.querySelector('[data-id="1"]')!.textContent).toBe('Count: 0');

        item.count = 5;
        update();

        expect(liveUl.querySelector('[data-id="1"]')!.textContent).toBe('Count: 5');
      });
    });
    ...
    describe("two independent lists in the same container", () => {
      it("updating one list does not affect the other", () => {
        ...
        const component = div(
          ul({ class: "list-1" }, list(() => list1Items, item => li(item))),
          ul({ class: "list-2" }, list(() => list2Items, item => li(item)))
        );
        ...
        expect(liveDiv.querySelector('.list-1')!.querySelectorAll('li').length).toBe(3);
        expect(liveDiv.querySelector('.list-2')!.querySelectorAll('li').length).toBe(2);

        list2Items = [];
        update();

        expect(liveDiv.querySelector('.list-1')!.querySelectorAll('li').length).toBe(3);
        expect(liveDiv.querySelector('.list-2')!.querySelectorAll('li').length).toBe(0);
      });
    });
    ...
    describe("list() SSR output consistency", () => {
      it("SSR renders all items correctly", () => {
        ...
        const html = renderToString(ul(list(() => items, item => li(item))));
      ...
      it("SSR renders an empty list as an empty ul", () => {
        const html = renderToString(ul(list(() => [], () => li("x"))));
        expect(html).toContain('<ul');
  ...
  describe("when() inside list() items", () => {
    it("each list item can have its own conditional branch", () => {
      ...
      expect(liveUl.querySelector('[data-id="1"]')!.textContent).toContain('in stock');
      expect(liveUl.querySelector('[data-id="2"]')!.textContent).toContain('out of stock');

      // Mark Gadget as back in stock
      products[1].inStock = true;
      update();

      expect(liveUl.querySelector('[data-id="2"]')!.textContent).toContain('in stock');
      expect(liveUl.querySelector('[data-id="2"]')!.textContent).not.toContain('out of stock');
    });

    it("a list item removed via list update also cleans its when() runtime", () => {
      interface Item { id: number; flagged: boolean; }
      let items: Item[] = [{ id: 1, flagged: true }, { id: 2, flagged: false }];

      const component = ul(
        list(() => items, it =>
          li({ "data-id": String(it.id) },
