...
describe("Dynamic List - Comment Markers", () => {
  ...
  describe("list function - comment markers", () => {
    ...
    it("should preserve siblings when rendering list", () => {
      ...
      expect((container.childNodes[0] as Element).textContent).toBe(
        "Before list",
      );
      ...
      expect((container.childNodes[6] as Element).textContent).toBe(
        "After list",
      );
    });
  ...
  describe("update function - reordering and preservation", () => {
    ...
    it("should add new items without recreating existing ones", () => {
      ...
      expect(container.querySelector('[data-item-id="1"]')).toBe(
        originalElement1,
      );
    });
    ...
    it("should keep duplicate object references in sync with the array order", () => {
      ...
      expect(
        initialElements.map((el) => el.getAttribute("data-item-id")),
      ).toEqual(["1", "2", "1"]);

      const duplicateNodes = Array.from(
        container.querySelectorAll('[data-item-id="1"]'),
      );
      ...
      expect(
        sortedElements.map((el) => el.getAttribute("data-item-id")),
      ).toEqual(["1", "1", "2"]);

      const updatedDuplicates = Array.from(
        container.querySelectorAll('[data-item-id="1"]'),
      );
  ...
  describe("multiple lists in same container", () => {
    it("should handle multiple independent lists", () => {
      const items1 = [
        { id: 1, name: "List1-Item1" },
        { id: 2, name: "List1-Item2" },
      ];
      const items2 = [
        { id: 1, name: "List2-Item1" },
        { id: 2, name: "List2-Item2" },
      ];
