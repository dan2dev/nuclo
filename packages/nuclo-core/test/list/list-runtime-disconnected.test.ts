...
describe('updateListRuntimes – disconnected markers (lines 190-200)', () => {
  ...
  it('keeps connected runtimes active and disconnects only dead ones', () => {
    ...
    createListRuntime(() => aliveItems, (item) => {
      const el = document.createElement('span') as unknown as ExpandedElement<ElementTagName>;
      (el as unknown as HTMLElement).textContent = item;
      return el;
    }, aliveHost, 0);
    ...
    expect((aliveHost as unknown as HTMLElement).querySelectorAll('span').length).toBe(2);
...
describe('updateListRuntimes – scope filtering', () => {
  it('skips runtime when scope does not contain startMarker', () => {
    ...
    expect((host as unknown as HTMLElement).querySelectorAll('span').length).toBe(1);
