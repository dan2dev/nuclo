...
export function createStyleQueries<
  const TDefinitions extends StyleQueryDefinitions,
>(queries: TDefinitions): StyleQueryBuilder<TDefinitions> {
  ...
  const queriesArray: Array<readonly [QueryKey, string]> = Array.isArray(
    queries,
  )
    ? ([...queries] as Array<readonly [QueryKey, string]>)
    : (Object.entries(queries) as unknown as Array<
    ...
    if (hasStyles) {
      ...
      for (const [key, styleBuilder] of Object.entries(styles ?? {})) {
        if (
          ...
          isPseudoClass(key) &&
          styleBuilder instanceof StyleBuilder
        ) {
          allQueryStyles.push({
            queryName: key,
      ...
      if (defaultStyles) {
        ...
        allStyleKeys[keyIndex++] =
          `default:${generateStyleKey(defaultStylesObj)}`;
      }
      ...
      const atRuleQueries: Array<{
        queryName: string;
        query: AtRuleQuery;
        styles: Record<string, string>;
      }> = [];
      const pseudoClassQueries: Array<{
        queryName: string;
        query: PseudoQuery;
        styles: Record<string, string>;
      }> = [];
