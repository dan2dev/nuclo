...
): (value?: string) => StyleBuilder {
  if (prop.isShorthand) {
    return () =>
      new StyleBuilder().add(prop.cssProperty, prop.defaultValue || "");
  } else {
    return (value?: string) =>
      new StyleBuilder().add(prop.cssProperty, value || "");
  }
}
