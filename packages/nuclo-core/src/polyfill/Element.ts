...
class SSRClassList {
  ...
  constructor(el: NucloElement) {
    this._el = el;
  }

  add(...tokens: string[]): void {
    const classes = this._el.className
      ? this._el.className.split(" ").filter(Boolean)
      : [];
  ...
  remove(...tokens: string[]): void {
    ...
    this._el.className = this._el.className
      .split(" ")
      .filter((c) => c && !tokens.includes(c))
      .join(" ");
  }
  contains(token: string): boolean {
    return (
      !!this._el.className && this._el.className.split(" ").includes(token)
    );
  }
  ...
  item(index: number): string | null {
    return this._el.className.split(" ").filter(Boolean)[index] ?? null;
  }
  get length(): number {
    return this._el.className
      ? this._el.className.split(" ").filter(Boolean).length
      : 0;
  }
  get value(): string {
    return this._el.className;
  ...
  forEach(
    cb: (value: string, key: number, parent: DOMTokenList) => void,
  ): void {
    const classes = this._el.className.split(" ").filter(Boolean);
    for (let i = 0; i < classes.length; i++)
      cb(classes[i], i, this as unknown as DOMTokenList);
  }
  ...
  keys(): Iterator<number> {
    ...
    return {
      next(): IteratorResult<number> {
        return i < len ? { value: i++, done: false } : { value: 0, done: true };
      },
    };
  }
...
function isTextNode(node: unknown): node is TextNode {
  return (
    ...
    node !== null &&
    "nodeType" in node &&
    (node as { nodeType: number }).nodeType === 3
  );
}
...
export class NucloElement extends NucloNode {
  ...
  insertBefore<T extends Node>(newNode: T, referenceNode: Node | null): T {
    ...
    if (index !== -1) {
      ...
      if (isBrowser && (this as any)["_childNodes"]) {
        const childNodesIndex = (this as any)["_childNodes"].indexOf(
          referenceNode,
        );
  ...
  dispatchEvent(event: Event): boolean {
    ...
    if (
      ...
      "dispatchEvent" in this.parentNode &&
