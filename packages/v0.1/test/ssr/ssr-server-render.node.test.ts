...
import { renderToString, renderToStringWithContainer } from "../../src/ssr/renderToString";
import "../../src/index";
...
describe("SSR — server render (Node.js)", () => {
  ...
  describe("static HTML output", () => {
    ...
    it("renders void elements as self-closing", () => {
      ...
      expect(renderToString(img({ src: "logo.png", alt: "Logo" }))).toBe('<img src="logo.png" alt="Logo" />');
    });
    ...
    it("produces a complete page structure suitable for server delivery", () => {
      const bodyContent = renderToString(
        main({ id: "app" },
          header(h1("My App")),
          article(
            p("Server-rendered with Nuclo v0.1")
          ),
          footer(small("© 2026"))
        )
      );
