...
/*
  This worker demonstrates per-request isolation using Deno Web Workers.
  It listens for a message that contains request details (like `requestPath`)
  and responds with either HTML for SSR or a JSON payload.
*/

self.onmessage = (e) => {
  const { requestPath, timestamp } = e.data;
  // For demonstration, if the main route is requested we return an HTML string
  if (requestPath === '/') {
    const worker1Url = new URL(`./worker1a.` + `ts`, import.meta.url).href
    import(worker1Url).then((module) => {
      const content = module.some;
      self.postMessage({ content });
      self.close();
    })

    // const html = `<div id="app"><h1>Hello from isolated worker!</h1><p>Generated at ${new Date(timestamp).toISOString()}</p></div>`;
    // self.postMessage({ html, timestamp })
  } else {
    const message = `Worker received path: ${requestPath} at ${new Date(timestamp).toISOString()}`;
    self.postMessage({ message, timestamp })
    self.close();
  }
  // Close worker after sending the response to ensure the worker's lifecycle matches a single request
  // self.close();
};
